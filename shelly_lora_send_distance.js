...
//Sends distance measure from (BLE event) every 60 seconds to avoid overusage of Lora limit!
...
function sendMessage(message) {
  ...
  Shelly.call(
    'Lora.SendBytes',
    { id: 110, data: btoa(encryptedMessage) },
...
Shelly.addEventHandler(
    function (event, ud) {
        if (typeof (event.info.data) !== 'undefined') { 
          if (typeof (event.info.data.distance) !== 'undefined') { 
             distance_data = JSON.stringify(event.info.data);
          }
        }
    },
    null
...
function startTimer() {
  alertTimer = Timer.set(60000,
      true,
      function (ud) {
          if (distance_data  !== 'undefined') {
            print("Lora distance sent",distance_data)
            sendMessage(distance_data); 
          }          
      },
      null
  );
